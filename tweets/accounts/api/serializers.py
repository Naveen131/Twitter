...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    def validate_password(self,value):
        if len(value)<getattr(settings,'PASSWORD_MIN_LENGTH',8):
            return serializers.ValidationError("Password sjouls be at least %s characters long"% getattr(settings, 'PASSWORD_MIN_LENGTH', 8))
        return value
    ...
    def create(self,validated_data):
        ...
        user = UserProfile.objects.create_user_profile(data=user_data,is_active=is_active,site=get_current_site(self.context['request']),send_email=True)
...
class UserLoginSerializer(serializers.ModelSerializer):
    ...
    def validate(self,data):
        ...
        if not email and not username:
            raise ValidationError("Please enter username or login again")
